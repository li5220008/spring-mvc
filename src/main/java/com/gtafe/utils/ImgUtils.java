...
public class ImgUtils {
    ...
    public static BufferedImage image = new BufferedImage(WIDTH, HEIGH, BufferedImage.TYPE_INT_RGB);
    /**
     * 随机生成验证码图片
     * @return <code>false</code> if no appropriate writer is found.
     */
    public static boolean randomImg(OutputStream output) {
        boolean flag = false;
        ...
        try {
            flag = ImageIO.write(image, "jpg", output);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return flag;
    ...
    private static void setBorder(Graphics g) {
        ...
        g.drawRect(1,1,WIDTH-2,HEIGH-2);
    }
    ...
    private static void drawRandomLine(Graphics g) {
        ...
        for(int i=0;i<5;i++){
            ...
            int y1 = new Random().nextInt(HEIGH);
            int x2 = new Random().nextInt(WIDTH);
            int y2 = new Random().nextInt(HEIGH);
            g.drawLine(x1,y1,x2,y2);
        }
    }

    private static void drawRandomNum(Graphics2D g) {
